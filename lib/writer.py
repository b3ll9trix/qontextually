...
def _upsert_source(
    ...
    *,
    ...
    cur = conn.execute(
        ...
        "VALUES (?, ?, ?, ?)",
        (document_path, source_type, raw_text, json.dumps(properties, ensure_ascii=False)),
    )
...
def _ensure_predicate(
    ...
    Returns the normalized predicate name that callers should use when
    writing the triple (so fresh case-variants land on the canonical row).
    """
    normalized = _normalize_predicate(predicate)
    row = conn.execute(
        "SELECT auto_added FROM predicates WHERE name = ?", (normalized,)
    ).fetchone()
    if row is None:
        conn.execute(
